  ...
  head: () => ({
    meta: [
      { title: "Проекты — Smart Group Kazakhstan" },
      {
        name: "description",
        content:
          "Портфолио реализованных проектов умного дома и автоматизации зданий в Казахстане.",
      },
...
const projects: P[] = [
  {
    city: "Астана",
    type: "Резиденция",
    title: "Резиденция на Мангилик Ел",
    area: "1 200 м²",
    scope: "Свет, климат, мультирум, охрана",
    grad: "cinematic-card",
  },
  {
    city: "Алматы",
    ...
    area: "2 400 м²",
    scope: "Инженерия, ландшафт, охрана периметра",
    grad: "cinematic-card-2",
  },
  {
    city: "Астана",
    type: "Апарт-отель",
...
function Projects() {
  ...
  return (
    <main className="pt-32">
      <section className="relative px-6 md:px-10 pb-20 overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          <Reveal
            as="h1"
            delay={120}
            className="text-[clamp(48px,9vw,160px)] font-extrabold tracking-tighter leading-[0.88] max-w-[18ch]"
          >
          ...
          {filters.map((x) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-[11px] uppercase tracking-[0.2em] font-bold transition-colors ${
                f === x ? "premium-button" : "text-mute hover:text-fg"
              }`}
            >
              {x}
            </button>
        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {list.map((p, i) => (
            <Reveal key={p.title} delay={i * 60} variant="fade">
              <Link
                to="/contact"
                className={`group block aspect-[4/5] relative overflow-hidden rounded-2xl ${p.grad} border border-white/5 hover:border-white/20 transition-colors`}
              >
                <div className="absolute inset-0 p-8 flex flex-col justify-between">
                  <div className="flex items-start justify-between">
                    ...
                    <ArrowUpRight className="size-4 text-mute group-hover:text-silver group-hover:rotate-12 transition-all" />
                  </div>
                  <div>
